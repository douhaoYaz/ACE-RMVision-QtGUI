...
void AngleSolver::getRotationSpeed() {
    ...
    if(data.sample_count < 15){
        ...
        if(data.sample_count > 0){
            timer_buff.end();
            float delta_t = timer_buff.calculateAverageTime();
            timer_buff.start();

            float theta_rotate = data.angle_buff - data.angle_buff_last;
            float spd = theta_rotate * 2 * CV_PI /360;
        ...
        if(point_coefficient.x < 0){
            data.orig_t = (CV_PI/1.884) - data.orig_t;
        }

        timer_buff.end();
        float real_t = data.orig_t + timer_buff.calculateAverageTime();
