...
void BuffFlabellum::getStatus(cv::Mat &img) {
    ...
    //对延伸出来的距离再进行调整
    offset_height.x *= 1.2f;
    offset_height.y *= 1.2f;
...
#ifdef IMSHOW_2_ROI
    cv::rectangle(img, rect_l, cv::Scalar(255), 1);
    cv::rectangle(img, rect_r, cv::Scalar(255), 1);
    cv::putText(img, std::to_string(intensity_l), pt_cen_l,
            cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(128, 0, 128));
    cv::putText(img, std::to_string(intensity_r), pt_cen_r,
            cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(128, 0, 128));
...
//    cv::dilate(img, img, element);
    ...
    if (param.other.color_enemy != 0)
        cv::subtract(channels_bgr[2], channels_bgr[0], data.img_color);
	else
        cv::subtract(channels_bgr[0], channels_bgr[2], data.img_color);
...
//        return 0;
    ...
    cv::threshold(data.img_color, data.img_color, param.buff.th_color, 255, cv::THRESH_BINARY);
...
//    cv::imshow("mask", img_bin);
    ...
    //提取所有轮廓，并建立双层结构关系
    cv::findContours(data.img_color, contours, hierarchy, cv::RETR_CCOMP, cv::CHAIN_APPROX_NONE);
    ...
    for (size_t i = 0; i < contours.size(); i++) {
        ...
        if (hierarchy[i][3] < 0 && contours[i].size() >= 6
                && hierarchy[i][2] < 6) {
            if (cv::arcLength(contours[i], true) < 60 || cv::contourArea(contours[i]) < 200) {
                cv::Rect rect_r = cv::boundingRect(contours[i]);
                cv::rectangle(data.img_show, rect_r, { 0xFF, 0, 0 }, 2);
            }
        }
...
//#endif
        ...
        flabellum.getStatus(data.img_color);
...
//#ifdef DEBUG_PUT_TEST_TARGET
        cv::putText(data.img_show, "<<---final target",
                cv::Point2f(5,5)+ target_final.rrect_small.center,
